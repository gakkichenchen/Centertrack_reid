...
def main(opt):
  ...
  Dataset = get_dataset(opt.dataset)#the holo_tracking.Holo_tracking
  res=Dataset(opt,'train')
  ...
  for epoch in range(start_epoch + 1, opt.num_epochs + 1):
    ...
    if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
      save_model(os.path.join(opt.save_dir, 'model1_id_1w_{}3.pth'.format(mark)), 
      ...
      save_model(os.path.join(opt.save_dir, 'model1_id_1w_last3.pth'), 
                 epoch, model, optimizer)
    logger.write('\n')
    if epoch in opt.save_point:
      save_model(os.path.join(opt.save_dir, 'model1_id_1w_{}3.pth'.format(epoch)), 
                 epoch, model, optimizer)
