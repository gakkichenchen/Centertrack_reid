...
class PrefetchDataset(torch.utils.data.Dataset):
  def __init__(self, opt, dataset, pre_process_func):
    #pre_process_func:<bound method Detector.pre_process of <detector.Detector object at 0x7f0dad3890b8>>
    #dataset:<dataset.datasets.holo_tracking.Holo_tracking object at 0x7f0d3e243198>
  ...
  def __getitem__(self, index):
    ...
    #img_info:{'file_name': '42_2019.02.200215_le...47450.jpeg', 'frame_id': 1, 'id': 1, 'video_id': 1}
    img_path = os.path.join(self.img_dir, img_info['file_name'])
    #'/home/ubuntu/cc/scripts/CenterTrack1/image/test_img'
    ...
    for scale in opt.test_scales:
      ...
      #calib[[1200,0,width/2,0],
      #       [0,1200,height/2,0],
      #       [0,0,1,0]]
      #pre_process_funs:detector.pre_process
      images[scale], meta[scale] = self.pre_process_func(
        image, scale, input_meta)
      #images[1]:[1,3,384inpheight,1280inp_width]
      #meta[1]:dict_keys(['calib', 'c', 's', 'height', 'width', 'out_height', 'out_width', 'inp_height', 'inp_width', 'trans_input', 'trans_output'])
...
def prefetch_test(opt):
  ...
  for ind, (img_id, pre_processed_images) in enumerate(data_loader):
    #pre_processed_images::dict_keys(['images', 'image', 'meta', 'is_first_frame', 'video_id'])
    if ind >= num_iters:
      break
